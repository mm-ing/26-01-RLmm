...
class CliffWalkerGUI:
    def __init__(self, master: tk.Tk):
        ...
        self.method_vars = {m.name: tk.BooleanVar(value=True) for m in self.methods}
    ...
    def _build_ui(self):
        ...
        for m in self.methods:
            ttk.Checkbutton(control_frame, text=m.name, variable=self.method_vars[m.name]).grid(row=row_idx, column=2, sticky="w", padx=(12, 0))
            row_idx += 1

        ttk.Checkbutton(control_frame, text="Compare methods", variable=self.compare_methods).grid(row=5, column=2, sticky="w", padx=(12, 0))
    ...
    def train_and_run(self):
        selected = [m.name for m in self.methods if self.method_vars[m.name].get()]
        if not selected:
        ...
        self._render_method = selected[0]
    ...
    def _train_worker(self, method_name: str, stop_event: threading.Event, enable_render: bool):
        ...
        env = CliffWalkingEnv(render_mode=render_mode)
