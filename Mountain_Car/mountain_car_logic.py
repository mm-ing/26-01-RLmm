...
class MountainCarEnv:
    def __init__(
        ...
        self.env_name = env_name or "MountainCarContinuous-v0"
        self.action_bins = max(2, int(action_bins))
        try:
            self.env = gym.make(self.env_name, render_mode=render_mode, goal_velocity=goal_velocity)
        except TypeError:
            self.env = gym.make(self.env_name, render_mode=render_mode)
        if self.env_name == "MountainCar-v0":
            if not isinstance(self.env.action_space, spaces.Discrete):
                raise ValueError("MountainCar-v0 action space must be Discrete.")
            self.action_bins = int(self.env.action_space.n)
            self.action_values = None
        else:
            if not isinstance(self.env.action_space, spaces.Box):
                raise ValueError("MountainCarContinuous action space must be Box.")
        ...
        if not isinstance(self.env.observation_space, spaces.Box):
            raise ValueError("MountainCar observation space must be Box.")
        if seed is not None:
    ...
    def _apply_init_state(self, obs: np.ndarray) -> np.ndarray:
        ...
        if self.x_init is not None:
            x_val = float(self.x_init)
            if low is None or high is None or (low[0] <= x_val <= high[0]):
                state[0] = x_val
        if self.y_init is not None:
            y_val = float(self.y_init)
            if low is None or high is None or (low[1] <= y_val <= high[1]):
                state[1] = y_val

        if low is not None and high is not None:
            state = np.clip(state, low, high)
        if hasattr(self.env, "unwrapped") and hasattr(self.env.unwrapped, "state"):
            setattr(self.env.unwrapped, "state", np.array(state, dtype=np.float32))
        return np.array(state, dtype=np.float32)

    def reset(self) -> np.ndarray:
        obs, _info = self.env.reset()
        obs = np.asarray(obs, dtype=np.float32)
        return self._apply_init_state(obs)
    ...
    def step(self, action):
        ...
        if self.reward_shaping:
            pos = float(obs[0])
            vel = float(obs[1])
            shaped_reward += 0.1 * abs(vel) + 0.5 * (pos + 0.5)
        return np.asarray(obs, dtype=np.float32), shaped_reward, bool(done), info
...
class Agent:
    def __init__(self, env: MountainCarEnv, policy: BasePolicy):
        self.env = env
        self.policy = policy
