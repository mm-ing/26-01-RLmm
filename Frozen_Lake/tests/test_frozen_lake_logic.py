...
def test_env_step_returns_values():
    env = FrozenLakeEnv(map_name="4x4", is_slippery=False, seed=0)
    state = env.reset()
...
def test_qlearning_update_changes_q():
    policy = QLearningPolicy(n_states=4, n_actions=2, alpha=0.5, gamma=0.9, eps_start=0.0, eps_end=0.0, total_episodes=1)
    policy.update(0, 1, 1.0, 0, True)
    assert policy.q[0, 1] > 0.0


def test_sarsa_update_changes_q():
    policy = SarsaPolicy(n_states=4, n_actions=2, alpha=0.5, gamma=0.9, eps_start=0.0, eps_end=0.0, total_episodes=1)
    policy.update(0, 0, 1.0, 1, True, next_action=0)
    assert policy.q[0, 0] > 0.0


def test_expected_sarsa_update_changes_q():
    policy = ExpectedSarsaPolicy(n_states=4, n_actions=2, alpha=0.5, gamma=0.9, eps_start=0.0, eps_end=0.0, total_episodes=1)
