...
class TaxiGUI:
    def __init__(self, master: tk.Tk, env: TaxiEnv, agent: Agent, policies: List):
        self.master = master
        self.env = env
        self.agent = agent
        self.policies = policies
        self.master.title("Taxi RL Demo")

        self.methods = [
            MethodConfig("Q-learning", QLearningPolicy, "#ffb3b3", "#ff4d4d"),
            MethodConfig("SARSA", SarsaPolicy, "#b3ffb3", "#4dff4d"),
            MethodConfig("Expected SARSA", ExpectedSarsaPolicy, "#b3d1ff", "#4d79ff"),
        ]
    ...
    def _build_ui(self):
        ...
        ttk.Label(control_frame, text="Step delay (s)").grid(row=5, column=0, sticky="w")
        self.step_delay_var = tk.DoubleVar(value=0.0)
        ttk.Entry(control_frame, textvariable=self.step_delay_var, width=8).grid(row=5, column=1)

        ttk.Label(control_frame, text="Max steps").grid(row=6, column=0, sticky="w")
        self.max_steps_var = tk.IntVar(value=200)
        ttk.Entry(control_frame, textvariable=self.max_steps_var, width=8).grid(row=6, column=1)

        ttk.Label(control_frame, text="Method").grid(row=0, column=2, sticky="w", padx=(12, 0))
        ttk.Combobox(control_frame, values=[m.name for m in self.methods], textvariable=self.method_var, state="readonly").grid(row=0, column=3, sticky="w")

        ttk.Checkbutton(control_frame, text="Compare methods", variable=self.compare_methods).grid(row=1, column=2, columnspan=2, sticky="w", padx=(12, 0))
        ttk.Checkbutton(control_frame, text="Is Raining", variable=self.is_raining_var).grid(row=2, column=2, columnspan=2, sticky="w", padx=(12, 0))
    ...
    def _train_worker(self, method_name: str, stop_event: threading.Event, enable_render: bool):
        ...
        is_raining = bool(self.is_raining_var.get())

        render_mode = "rgb_array" if enable_render else None
        env = TaxiEnv(is_raining=is_raining, render_mode=render_mode)
        ...
        policy = policy_cls(n_states, n_actions, alpha, gamma, eps_start, eps_end, episodes)
        ...
        for ep in range(episodes):
            if stop_event.is_set():
                break
            total = agent.run_episode(ep, episodes, max_steps, step_delay, render_cb if enable_render else None, stop_event)
